...
module ots_staking::pts {
    ...
    public(package) fun mint(allow_cap: &AllowCap<PTS>, mintCap: &mut MintCap<PTS>, to: address,  amount: u64, ctx: &mut TxContext) {
    ...
    public(package) fun mint_reward( mintCap: &mut MintCap<PTS>,  amount: u64, ctx: &mut TxContext):Coin<PTS> {
        assert!(amount > 0, AmountLessThenzero);
       let MintCap{
            ..,
            treasury_cap,
    ...
    public(package) fun burn( mintCap: &mut MintCap<PTS>, coins: Coin<PTS>) {
          let MintCap{
            ..,
            treasury_cap,
       } = mintCap;
